...
class CSNN(nn.Module):
    def __init__(self, T: int, channels: int, use_cupy=False):
        ...
        layer.Conv2d(1, channels*2, kernel_size=5, padding=1, stride=1, bias=False),
        layer.BatchNorm2d(channels*2),
        neuron.IFNode(surrogate_function=surrogate.ATan()), # 26 * 26 * 2C AlexNet CL2
    
        layer.Conv2d(channels*2, channels*2, kernel_size=4, padding=1, stride=2, bias=False),
        layer.BatchNorm2d(channels*2),
        neuron.IFNode(surrogate_function=surrogate.ATan()), # 13 * 13 * 2C AlexNet Pool2

        layer.Conv2d(channels*2, channels*3, kernel_size=3, padding=1, stride=1, bias=False),
        layer.BatchNorm2d(channels*3),
        neuron.IFNode(surrogate_function=surrogate.ATan()), # 13 * 13 * 3C AlexNet CL3

        # layer.Conv2d(channels*3, channels*3, kernel_size=3, padding=1, stride=1, bias=False),
        # layer.BatchNorm2d(channels*3),
        # neuron.IFNode(surrogate_function=surrogate.ATan()), # 13 * 13 * 3C AlexNet CL4

        layer.Conv2d(channels*3, channels*2, kernel_size=3, padding=1, stride=1, bias=False),
        layer.BatchNorm2d(channels*2),
        neuron.IFNode(surrogate_function=surrogate.ATan()), # 13 * 13 * 2C AlexNet Pool2

        layer.Conv2d(channels*2, channels*2, kernel_size=3, padding=0, stride=2, bias=False),
        layer.BatchNorm2d(channels*2),
