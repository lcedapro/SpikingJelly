...
def main():
    # 加载原始权重文件
    checkpoint_path = './logs/T4_b768_sgd_lr0.1_c32_amp_cupy/checkpoint_max.pth'
    checkpoint = torch.load(checkpoint_path, map_location='cpu', weights_only=False)
    
    # 将BatchNorm层的参数吸收到Conv层中
    fused_checkpoint = checkpoint.copy()
    conv_prefix_list = ['conv_fc.0', 'conv_fc.3', 'conv_fc.6', 'conv_fc.9']
    bn_prefix_list = ['conv_fc.1', 'conv_fc.4', 'conv_fc.7', 'conv_fc.10']
    fused_checkpoint['net'] = manual_fuse_model_bn_to_conv(checkpoint['net'], conv_prefix_list, bn_prefix_list)

    # 保存新的权重文件
    fused_checkpoint_path = './logs/T4_b768_sgd_lr0.1_c32_amp_cupy/checkpoint_max_bn2conv.pth'
