...
# python .\infer_conv2int.py -data-dir ./ -out-dir ./logs -opt sgd -resume './logs/T4_b768_sgd_lr0.1_c32_amp_cupy/checkpoint_max_conv2int.pth' -b 16 -device cpu
# epoch = 0, test_loss = 0.0186, test_acc = 0.8827
# test speed = 843.2729 images/s
...
class CSNN(nn.Module):
    def __init__(self, T: int, channels: int, use_cupy=False):
        ...
        neuron.IFNode(surrogate_function=surrogate.ATan(), v_threshold=35.0),  # 26 * 26
        layer.Conv2d(channels, channels*2, kernel_size=3, padding=0, stride=2, bias=True),
        # layer.BatchNorm2d(channels*2),
        neuron.IFNode(surrogate_function=surrogate.ATan(), v_threshold=73.0),  # 12 * 12

        layer.Conv2d(channels*2, channels*2, kernel_size=3, padding=0, stride=1, bias=True),
        # layer.BatchNorm2d(channels*2),
        neuron.IFNode(surrogate_function=surrogate.ATan(), v_threshold=73.0),  # 10 * 10
        layer.Conv2d(channels*2, channels*4, kernel_size=3, padding=0, stride=2, bias=True),
        # layer.BatchNorm2d(channels*4),
        neuron.IFNode(surrogate_function=surrogate.ATan(), v_threshold=65.0),  # 4 * 4

        layer.Flatten(),
        layer.Linear(channels*4*4*4, channels*2*4*4, bias=False),
        neuron.IFNode(surrogate_function=surrogate.ATan(), v_threshold=2944.0),

        layer.Linear(channels*2*4*4, 10, bias=False),
        neuron.IFNode(surrogate_function=surrogate.ATan(), v_threshold=996.0),
        )
