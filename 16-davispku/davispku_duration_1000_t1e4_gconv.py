...
class PythonNet(nn.Module):
    def __init__(self, channels: int):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(1, 2, kernel_size=7, padding=2, stride=2, bias=False), nn.BatchNorm2d(2),
            neuron.IFNode(surrogate_function=surrogate.ATan(), detach_reset=True),
            nn.Conv2d(2, 4, kernel_size=7, padding=3, stride=2, bias=False), nn.BatchNorm2d(4),
            neuron.IFNode(surrogate_function=surrogate.ATan(), detach_reset=True),
            nn.Conv2d(4, 4, kernel_size=5, padding=2, stride=1, groups=2, bias=False), nn.BatchNorm2d(4),
            neuron.IFNode(surrogate_function=surrogate.ATan(), detach_reset=True),
        )
        self.fc = nn.Sequential(
            nn.Flatten(),
            layer.Dropout(0.5),
            nn.Linear(channels*4 * 21 * 16, channels*8 * 8 * 8, bias=False),
            neuron.IFNode(surrogate_function=surrogate.ATan(), detach_reset=True),
            layer.Dropout(0.5),
