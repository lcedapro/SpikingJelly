...
def test(test_num: int = 100):
    # 结果csv初始化
    with open('paibox_spikingjelly_compare_main_897_result.csv', 'w', newline='') as file:
    ...
    for i, (image_tensor, label_tensor) in enumerate(test_data_loader):
        ...
        # 将结果写入csv
        with open('paibox_spikingjelly_compare_main_897_result.csv', 'a', newline='') as file:
            writer = csv.writer(file)
