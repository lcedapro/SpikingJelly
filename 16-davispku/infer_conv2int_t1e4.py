# py .\infer_conv2int_t1e4.py -data_dir ./ -out_dir ./logs -channels 2 -resume './logs_t1e4_gconv/T_4_b_16_c_2_SGD_lr_0.2_CosALR_48_amp_cupy/checkpoint_max_conv2int.pth'
# epoch=0, test_loss=0.013099501098622568, test_acc=0.93359375, max_test_acc=0, total_time=12.539369106292725
...
vthr_list = np.load('./logs_t1e4_gconv/T_4_b_16_c_2_SGD_lr_0.2_CosALR_48_amp_cupy/vthr_list.npy')
...
class PythonNet(nn.Module):
    def __init__(self, channels: int, vthr_list: list):
        ...
        # conv.extend(PythonNet.convpool5x5(channels*2, channels*4))
    ...
    @staticmethod
    def conv5x5(in_channels: int, out_channels):
        return [
    ...
    @staticmethod
    def convpool5x5(in_channels: int, out_channels):
        return [
            nn.Conv2d(in_channels, out_channels, kernel_size=5, padding=2, stride=2, bias=False),
            nn.BatchNorm2d(out_channels),
            neuron.IFNode(surrogate_function=surrogate.ATan(), detach_reset=True)
        ]
...
try:
    ...
    class CextNet(nn.Module):
        def __init__(self, channels: int):
            ...
            # conv.extend(CextNet.convpool5x5(channels*2, channels*4))
