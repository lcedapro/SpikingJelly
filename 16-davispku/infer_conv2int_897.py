# py .\infer_conv2int_897.py -data_dir ./ -out_dir ./logs -channels 2 -resume './logs_897_others/T_16_b_4_c_2_SGD_lr_0.4_CosALR_48_amp_cupy_random_en=True/checkpoint_max_conv2int.pth'
# epoch=0, test_loss=0.050305990036576986, test_acc=0.8125, max_test_acc=0, total_time=6.663844585418701
...
vthr_list = np.load('./logs_897/T_16_b_4_c_2_SGD_lr_0.4_CosALR_48_amp_cupy/vthr_list.npy')
...
class PythonNet(nn.Module):
    def __init__(self, channels: int, vthr_list: list):
        super().__init__()
        self.pool = nn.Sequential(nn.MaxPool2d(5, 5))
        conv = []
        ...
        self.fc = nn.Sequential(
            nn.Flatten(),
            layer.Dropout(0.5),
            nn.Linear(channels*4 * 18 * 13, channels*8 * 8 * 8, bias=False),
            ...
            neuron.IFNode(surrogate_function=surrogate.ATan(), detach_reset=True, v_threshold=vthr_list[5]),
            layer.Dropout(0.5),
...
try:
    ...
    class CextNet(nn.Module):
        def __init__(self, channels: int, vthr_list: list):
            super().__init__()
            self.pool = nn.Sequential(layer.SeqToANNContainer(nn.MaxPool2d(5, 5)))
            conv = []
            ...
            self.fc = nn.Sequential(
                ...
                layer.SeqToANNContainer(nn.Linear(channels*4 * 18 * 13, channels*8 * 8 * 8, bias=False)),
                neuron.MultiStepIFNode(surrogate_function=surrogate.ATan(), detach_reset=True, backend='cupy', v_threshold=vthr_list[4]),
                layer.MultiStepDropout(0.5),
                layer.SeqToANNContainer(nn.Linear(channels*8 * 8 * 8, channels*8 * 4 * 4, bias=False)),
                neuron.MultiStepIFNode(surrogate_function=surrogate.ATan(), detach_reset=True, backend='cupy', v_threshold=vthr_list[5]),
                layer.MultiStepDropout(0.5),
...
def main():
    ...
    * :ref:`中文API <classify_dvsg.__init__-cn>`
    ...
    test_data_loader = DataLoader(test_dataset, batch_size=args.b, shuffle=False, num_workers=args.j, drop_last=False, pin_memory=True)
