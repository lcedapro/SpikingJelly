# py .\infer_conv2int.py -data_dir ..\15-classify_dvsg\DVS128Gesture\ -out_dir ./logs -channels 32 -resume './logs/T_16_b_48_c_32_Adam_lr_0.01_CosALR_16_amp_cupy_3/checkpoint_max_conv2int.pth'
# epoch=0, test_loss=0.03015176202946653, test_acc=0.78125, max_test_acc=0, total_time=28.006732940673828

vthr_list = [53,146,134,118,124,97,151,76,113,99,121,90]
...
class PythonNet(nn.Module):
    def __init__(self, channels: int, vthr_list: list[float]):
        super().__init__()
        conv = []
        conv.extend(PythonNet.conv3x3(2, channels, vthr=vthr_list[0]))
        conv.extend(PythonNet.convpool2x2(channels, channels, vthr=vthr_list[1]))
        for i in range(4):
            conv.extend(PythonNet.conv3x3(channels, channels, vthr=vthr_list[2 + 2 * i]))
            conv.extend(PythonNet.convpool2x2(channels, channels, vthr=vthr_list[2 + 2 * i + 1]))
        self.conv = nn.Sequential(*conv)
        self.fc = nn.Sequential(
            nn.Flatten(),
            layer.Dropout(0.5),
            nn.Linear(channels * 4 * 4, channels * 2 * 2, bias=False),
            neuron.IFNode(surrogate_function=surrogate.ATan(), detach_reset=True, v_threshold=vthr_list[2 + 2 * 4]),
            layer.Dropout(0.5),
            nn.Linear(channels * 2 * 2, 110, bias=False),
            neuron.IFNode(surrogate_function=surrogate.ATan(), detach_reset=True, v_threshold=vthr_list[2 + 2 * 4 + 1])
        )
    ...
    @staticmethod
    def conv3x3(in_channels: int, out_channels, vthr: float = 1.0):
        return [
            nn.Conv2d(in_channels, out_channels, kernel_size=3, padding=1, bias=True),
            # nn.BatchNorm2d(out_channels),
            neuron.IFNode(surrogate_function=surrogate.ATan(), detach_reset=True, v_threshold=vthr)
        ]
    
    @staticmethod
    def convpool2x2(in_channels: int, out_channels, vthr: float = 1.0):
        return [
...
try:
    ...
    class CextNet(nn.Module):
        def __init__(self, channels: int, vthr_list: list[float]):
            super().__init__()
            conv = []
            conv.extend(CextNet.conv3x3(2, channels, vthr=vthr_list[0]))
            conv.extend(CextNet.convpool2x2(channels, channels, vthr=vthr_list[1]))
            for i in range(4):
                conv.extend(CextNet.conv3x3(channels, channels, vthr=vthr_list[2 + 2 * i]))
                conv.extend(CextNet.convpool2x2(channels, channels, vthr=vthr_list[2 + 2 * i + 1]))
            self.conv = nn.Sequential(*conv)
            self.fc = nn.Sequential(
                nn.Flatten(2),
                layer.MultiStepDropout(0.5),
                layer.SeqToANNContainer(nn.Linear(channels * 4 * 4, channels * 2 * 2, bias=False)),
                neuron.MultiStepIFNode(surrogate_function=surrogate.ATan(), detach_reset=True, backend='cupy', v_threshold=vthr_list[2 + 2 * 4]),
                layer.MultiStepDropout(0.5),
                layer.SeqToANNContainer(nn.Linear(channels * 2 * 2, 110, bias=False)),
                neuron.MultiStepIFNode(surrogate_function=surrogate.ATan(), detach_reset=True, backend='cupy', v_threshold=vthr_list[2 + 2 * 4 + 1])
            )
            self.vote = VotingLayer(10)
        ...
        def conv3x3(in_channels: int, out_channels, vthr: float = 1.0):
            return [
                ...
                neuron.MultiStepIFNode(surrogate_function=surrogate.ATan(), detach_reset=True, backend='cupy', v_threshold=vthr)
            ]
