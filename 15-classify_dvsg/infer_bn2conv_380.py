# py .\infer_bn2conv_380.py -data_dir ..\15-classify_dvsg\DVS128Gesture\ -out_dir ./logs -channels 4 -resume './logs_380/T_16_b_256_c_4_SGD_lr_0.2_CosALR_48_amp_cupy-4/checkpoint_max_bn2conv.pth'
# epoch=0, test_loss=0.03205444280885988, test_acc=0.8194444444444444, max_test_acc=0, total_time=14.450466871261597
...
class PythonNet(nn.Module):
    def __init__(self, channels: int):
        ...
        conv.extend(PythonNet.convpool5x5(channels*4, channels*8))
        conv.extend(PythonNet.conv5x5(channels*8, channels*8))
...
try:
    ...
    class CextNet(nn.Module):
        def __init__(self, channels: int):
            ...
            conv.extend(CextNet.convpool5x5(channels*4, channels*8))
            conv.extend(CextNet.conv5x5(channels*8, channels*8))
            self.conv = nn.Sequential(*conv)
            self.fc = nn.Sequential(
                nn.Flatten(2),
